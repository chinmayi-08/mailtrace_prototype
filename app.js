...
function shell(){
  ...
  $('#app').innerHTML=`<div class="shell"><aside class="sidebar"><div class="brand"><div class="logo">⌁</div><div><b>MAILTRACE</b><small>SIH26106 · CYBER FORENSICS</small></div></div><nav id="nav">${[['dashboard','▦','dashboard'],['analysis','◈','analyze'],['cases','◫','investigations'],['correlation','⌘','correlation'],['intel','◎','intel'],['reports','▤','reports'],['settings','⚙','settings'],['help','?','help']].map(([v,ic,k])=>`<a data-view="${v}" class="${state.view===v?'active':''}"><i>${ic}</i><span>${esc(t(k))}</span></a>`).join('')}</nav><div class="side-foot"><span class="live-dot"></span> ${t('localFirst')}<br><small>Defensive demo · safe handling</small></div></aside><main class="main"><header class="top"><div><div class="eyebrow">SMART INDIA HACKATHON 2026 · SIH26106</div><h1 id="pageTitle"></h1><p id="pageSub"></p></div><div class="top-actions"><button class="btn ghost" id="themeBtn">${state.theme==='dark'?'☀':'☾'}</button><select class="select" id="langSelect" aria-label="language">${Object.entries(CONFIG.languages).map(([k,n])=>`<option value="${k}" ${k===state.lang?'selected':''}>${n}</option>`).join('')}</select><button class="btn" id="trySafe">${t('trySafe')}</button><button class="btn primary" id="tryPhish">${t('tryPhish')}</button></div></header><section id="content"></section></main></div><div id="toast" class="toast hidden"></div>`;
  bindShell(); setHeader(); renderView();
}
function bindShell(){
  $('#nav').onclick=e=>{const a=e.target.closest('a');if(!a)return;state.view=a.dataset.view;state.tab='overview';renderView();};
