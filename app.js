...
const DEMO_DMARC_FAIL = `From: Billing Notice <billing@trusted-example.edu>\nTo: analyst@example.com\nReply-To: billing@trusted-example.edu\nReturn-Path: <mailer@transaction-service.test>\nSubject: Account statement notification\nDate: Thu, 11 Sep 2026 10:20:00 +0530\nMessage-ID: <demo-dmarc-fail-26106@mailtrace.local>\nAuthentication-Results: mx.example.edu; spf=pass smtp.mailfrom=transaction-service.test; dkim=pass header.d=transaction-service.test; dmarc=fail header.from=trusted-example.edu\nContent-Type: text/plain\n\nHello,\n\nYour monthly account statement is ready for review. Please contact the billing team using your known support channel if you have questions.\n\nRegards,\nBilling Operations`;
...
function addDashboardWorkflow(){
  const stats=$('#content .stats'); if(!stats) return;
  stats.insertAdjacentHTML('afterend',`<div class="workflow card"><div><span class="label">HOW MAILTRACE WORKS</span><b>From email evidence to an investigation-ready report</b></div><ol><li><i>1</i>Upload .eml</li><li><i>2</i>Extract evidence</li><li><i>3</i>Score risk</li><li><i>4</i>Correlate indicators</li><li><i>5</i>Generate report</li></ol></div>`);
  const demos=$('#quickDrop .demo-links'); if(demos){ demos.insertAdjacentHTML('beforeend','<button id="authMismatchDemo">Try DMARC Mismatch</button>'); $('#authMismatchDemo').onclick=runAuthMismatchDemo; }
}

function addCaseContext(c){
  const hero=$('.hero-case > div:first-child');
  if(c.demo && hero) hero.insertAdjacentHTML('beforeend','<span class="demo-badge">DEMO DATA · APPROXIMATE INTELLIGENCE</span>');
  const box=$('.classification')?.closest('.box'); if(!box) return;
  const weights=[
    ['Reply-To mismatch',/Reply-To address differs/,24],['Domain mismatch',/Sender and Reply-To domains/,12],['URLs in message',/URL.*found in the message body/,Math.min(15,c.urls.length*6)],['Urgency language',/Urgency-based/,18],['SPF failure',/SPF authentication failed/,10],['DKIM failure',/DKIM authentication failed/,8],['DMARC failure',/DMARC authentication/,10],['Public IP indicator',/public IP indicator/,Math.min(8,c.ips.filter(ip=>!isPrivateIp(ip)).length*4)],['Attachment present',/attachment.*present/,Math.min(10,c.attachments.length*5)],['Suspicious domain pattern',/Domain naming/,14],['Credential language',/Credential or sign-in/,7]
  ].filter(([,pattern])=>c.reasons.some(reason=>pattern.test(reason)));
  if(!weights.length) return;
  box.insertAdjacentHTML('beforeend',`<div class="score-breakdown"><div class="label">WHY ${c.score}/100?</div>${weights.map(([name,,points])=>`<div><span>${name}</span><b>+${points}</b></div>`).join('')}<small>Score uses explainable local rules; it is not a certified ML verdict.</small></div>`);
}

function addAuthenticationContext(c){
  const grid=$('.auth-grid'); if(!grid) return;
  let text='Authentication results are incomplete. Treat the email as a lead and review the original headers.';
  if(c.auth.spf==='PASS'&&c.auth.dkim==='PASS'&&c.auth.dmarc==='PASS') text='All three checks passed. This supports the sender identity, but message content and links should still be reviewed.';
  if(c.auth.spf==='PASS'&&c.auth.dkim==='PASS'&&c.auth.dmarc==='FAIL') text='SPF and DKIM passed, but DMARC failed. The sending service was authorized and signed the message, yet the visible From domain did not align with that authenticated identity. Review before trusting it.';
  if(c.auth.dmarc==='FAIL' && !(c.auth.spf==='PASS'&&c.auth.dkim==='PASS')) text='DMARC failed alongside another authentication failure. This is a stronger spoofing or configuration warning.';
  grid.insertAdjacentHTML('afterend',`<div class="auth-insight"><b>Authentication interpretation</b><span>${text}</span></div>`);
}
function dashboard(){
...
function geoView(){
  setHeader(); if(!state.current){$('#content').innerHTML=`<div class="card empty">${t('noCurrent')}</div>`;return;}
  const c=state.current, mapped=c.geolocation.filter(g=>g.latitude!==null), hasIp=c.geolocation.length>0;
  const markerMarkup=mapped.map(g=>`<div class="marker" style="left:${Math.max(8,Math.min(92,(g.longitude+180)/360*100))}%;top:${Math.max(8,Math.min(86,(90-g.latitude)/180*100))}%" title="${esc(g.ip)}"></div>`).join('');
  const emptyHint=!hasIp?'<div class="geo-empty"><b>No public IP found in this email.</b><span>Geolocation needs a public relay IP from the email headers. Try the Phishing Demo to view sample map data.</span></div>':'';
  $('#content').innerHTML=`<div class="card"><div class="section-head"><div><b>Approximate IP geolocation</b><span>${c.demo?'DEMO / FALLBACK INTELLIGENCE':hasIp?'PROVIDER REQUIRED':'NO PUBLIC IP'}</span></div></div><div class="mapbox"><div class="mapgrid"></div><div class="world"></div>${markerMarkup}${emptyHint}<div class="map-label">⚠ ${t('geoNote')}</div></div></div><div class="grid three section">${c.geolocation.map(g=>`<div class="card box"><div class="label">IP INDICATOR</div><h3>${esc(g.ip)}</h3><div class="kv"><div>Country</div><div>${esc(g.country)}</div><div>City</div><div>${esc(g.city)}</div><div>ISP</div><div>${esc(g.isp)}</div><div>ASN</div><div>${esc(g.asn)}</div><div>Source</div><div>${esc(g.source)}</div></div></div>`).join('')||`<div class="card empty"><b>No external IPs were extracted.</b><br>Upload an email with a public relay IP or use the Phishing Demo for sample geolocation.</div>`}</div>`;
}
...
function renderView(){
  ...
  if(state.view==='dashboard') addDashboardWorkflow();
  if((state.view==='analysis'||state.view==='case') && state.current){ addCaseContext(state.current); addAuthenticationContext(state.current); }
...
async function runAuthMismatchDemo(){ const c=await analyzeEmail(DEMO_DMARC_FAIL,'demo-spf-dkim-pass-dmarc-fail.eml','demo',true); saveCurrent(c); state.current=c; state.view='analysis'; renderView(); toast('DMARC mismatch demo loaded'); }
